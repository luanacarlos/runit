  ...
  const exportToExcel = async (raceId, raceName) => {
    ...
    inscritos.forEach(inscrito => {
      worksheet.addRow({
        ...
        kit: inscrito.kitOption === "delivery" ? "Entrega" : "Retirar",
        address: inscrito.address || ""
      });
    });
      ...
      <div className="p-6">
        ...
        <div className="max-w-6xl mx-auto mb-8">
          ...
          <p className="text-gray-600 mt-2">
            Gerencie corridas e veja as inscrições
          </p>
        </div>
            ...
            <div className="bg-white rounded-lg shadow-xl max-w-lg w-full p-6">
              ...
              <form onSubmit={handleCreateCorrida} className="space-y-4">
                <input
                  ...
                  onChange={(e) =>
                    setFormCorrida({ ...formCorrida, nome: e.target.value })
                  }
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormCorrida({ ...formCorrida, data: e.target.value })
                  }
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-600 focus:border-transparent"
                  required
                />
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormCorrida({ ...formCorrida, local: e.target.value })
                  }
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-600 focus:border-transparent"
                  required
                />
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormCorrida({ ...formCorrida, imagem: e.target.value })
                  }
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-600 focus:border-transparent"
                />
        ...
        <div className="max-w-6xl mx-auto space-y-4">
          {corridas.length === 0 ? (
            <div className="bg-white rounded-lg shadow p-8 text-center text-gray-600">
              <p className="text-lg">
                Nenhuma corrida criada ainda. Crie uma para começar!
              </p>
            </div>
          ) : (
                ...
                {expandedRaceId === corrida._id && (
                  <div className="p-6 bg-gray-50">
                    <div className="flex justify-between items-center mb-4">
                      ...
                      <button
                        onClick={() =>
                          exportToExcel(corrida._id, corrida.nome)
                        }
                    ...
                    {inscrições[corrida._id]?.length === 0 ? (
                      <p className="text-gray-600 text-center py-8">
                        Nenhuma inscrição nesta corrida
                      </p>
                    ) : (
                        ...
                        <table className="w-full text-sm">
                          <thead>
                            <tr className="bg-gray-200">
                              ...
                              <th className="text-left p-3">Kit</th>
                              <th className="text-left p-3">Endereço</th>
                            </tr>
                          </thead>
                          <tbody>
                            {inscrições[corrida._id].map(
                              (inscrito, index) => (
                                <tr
                                  key={index}
                                  className={
                                ...
                                >
                                  ...
                                  <td className="p-3">
                                    {new Date(
                                      inscrito.inscrito_em
                                    ).toLocaleDateString("pt-BR")}
                                  </td>
                                  <td className="p-3">
                                    {inscrito.kitOption === "delivery"
                                      ? "Entrega"
                                      : "Retirar"}
                                  </td>
                                  <td className="p-3">
